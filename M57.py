...
r = Constant(0.19) # Taxa de crescimento de células tumorais.

gamma = Constant(1.8) # Taxa Citotóxica.

r_min = 0.001 # Taxa de expansão basal das células CAR-T.

p_1 = 4.765 # Taxa de Expansão inicial.

p_2 = 2.46e-3 # Modula a duração da taxa de expansão.

p_3 = 2.61 # Decaimento da taxa de expansão.

A = 1.5e4 # Constante de meia saturação da expansão das células CAR-T.

mu = 0.34 # Taxa de mortalidade de CT.

epsilon = 0.01 # Taxa de conversão de CT para CM.

theta_M = 1e-3 # Coeficiente de conversão de células CAR-T.

alpha = 1.189e-6 # Coeficiente de inativação/inibição de células CAR-T por células tumorais.

mu_M = 5e-3 # Taxa de mortalidade de células CAR-T de memória.

a = Constant(0) # Constantes de meia saturação da expansão da função de lise.
...
CT = 2.7e7 # Condição inicial das células CT.

CM = 0 # Condição inicial das células CM.

kI = Constant(9.479e8) # Constante de meia saturação da pressão da terapia.
...
tempo_guardar = [10, 40, 80, 120, 280] # Armazenamento de TP e TN nestes tempos (dias).
...
while t < Total_time: # Iterações no tempo.
	...
	while ((abs(integral_T - integral_T_ant))/integral_T) > tol and k < max_iterations: # Critério de Convergência.
		...
		integral_Ts = (assemble(conditional(gt(x, 0.42), TP_ant, 0)*dx)) # Cálculo da integral TS.
...
output_file = 'CART_total_M57.txt'
...
output_file = 'funcao_lise_M57.txt'
...
output_file = 'taxa_citoto_M57.txt'
