...
TP0 = Constant(1e7) # N° iniciais de TP (células positivas para antígenos).
...
TN0 = Constant(1) # N° iniciais de TN (células negativas para antígenos).
...
TN.interpolate((TN0/(dN * sqrt(Constant(2) * pi)) * exp(-Constant(0.5)*((x - xN)/dN)**Constant(2)))) # TP recebe este valor.
...
r = Constant(0.14) # Taxa de crescimento de células tumorais.

gamma = Constant(0.74) # Taxa Citotóxica.

r_min = 0.001 # Taxa de expansão basal das células CAR-T.

p_1 = 5.76 # Taxa de Expansão inicial.

p_2 =  3.24e-32# Modula a duração da taxa de expansão.

p_3 = 33 # Decaimento da taxa de expansão.

A = 6.5e5 # Constante de meia saturação da expansão das células CAR-T.

mu = 0.848 # Taxa de mortalidade de CT.

epsilon = 0.002 # Taxa de conversão de CT para CM.

theta_M = 1e-9 # Coeficiente de conversão de células CAR-T.

alpha = 9.2e-7 # Coeficiente de inativação/inibição de células CAR-T por células tumorais.

mu_M = 0.01 # Taxa de mortalidade de células CAR-T de memória.

a = Constant(6e6) # Constantes de meia saturação da expansão da função de lise.
...
CT = 4.68e8 # Condição inicial das células CT.

CM = 0 # Condição inicial das células CM.

kI = Constant(3.06e9) # Constante de meia saturação da pressão da terapia.
...
while t < Total_time: # Iterações no tempo.
	...
	# Implementação do método do Euler explícito:
	...
	while ((abs(integral_T - integral_T_ant))/integral_T) > tol and k < max_iterations: # Critério de Convergência.
		...
		TP_ant = TPsol # Atualizará TP_ant com a solução obtida na formulação variacional (TPsol), transformada em vetor;

		TN_ant = TNsol # Atualizará TP_ant com a solução obtida na formulação variacional (TNsol), transformada em vetor;
	...
	if t >= passo_tempo:  # Guardar somente a cada passo 0.1 os seguintes resultados:
		...
		CART_total.append(total_Cart)  
		lise_lista.append(lise_atual) 
		citotox_lista.append(taxa_citotoxidade_valores) 
...
output_file = 'CART_total_B10.txt'
...
output_file = 'funcao_lise_B10.txt'
...
output_file = 'taxa_citoto_B10.txt'
